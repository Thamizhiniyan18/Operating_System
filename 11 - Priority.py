...
def sorting(given_list):
    ...
    for i in range(n - 1):
        for j in range(n - 1):
            if given_list[j][2] < given_list[j + 1][2]:
                temp = given_list[j]
...
def main():
    ...
    print("Enter the arrival time, burst time and priority separated by a space [ Eg : arrival_time burst_time priority]")

    # Getting the processes and its arrival_time, burst_time, priority
    for i in range(number_of_processes):
        process_table.append(input().split(' '))

    sorting(process_table)
