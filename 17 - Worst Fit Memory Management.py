def main():
    ...
    for i in range(len(process_size)):
        ...
        for j in range(len(memory_block_size)):
            if memory_block_size[j] >= process_size[i]:
                if worst_fit == -1:
                    worst_fit = j
                elif memory_block_size[worst_fit] < memory_block_size[j]:
                    worst_fit = j

        if worst_fit != -1:
            # Allocating the block j to the current process
            allocated_memory_block[i] = worst_fit

            # Reduce available memory in this block
            memory_block_size[worst_fit] -= process_size[i]
    ...
    for i in range(len(process_size)):
        print(
            ...
            f"{allocated_memory_block[i] + 1}".center(20) if allocated_memory_block[i] != -1 else "Not Allocated".center(20),
            f"{memory_block_size[allocated_memory_block[i]]}".center(20) if allocated_memory_block[i] != -1 else "Not Allocated".center(20)
        )
